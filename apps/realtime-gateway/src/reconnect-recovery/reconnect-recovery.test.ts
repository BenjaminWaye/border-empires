...
import { RECONNECT_COMMAND_TYPES } from "../../../../packages/sim-protocol/src/command-coverage-sets/command-coverage-sets.js";
...
describe("buildInitMessage", () => {
  it("keeps reconnect recovery empty while preserving nextClientSeq across the full phase-4 command surface", async () => {
    ...
    expect(init.techCatalog).toEqual(expect.arrayContaining([expect.objectContaining({ id: "agriculture", name: "Hyperfeed Seedstock Consignment" })]));
    ...
    const snapshotBootstrap: LegacySnapshotBootstrap = {
      ...
      playerProfiles: new Map([
        [
          "player-1",
          {
            ...
            economyBreakdown: { GOLD: { sources: [], sinks: [] }, FOOD: { sources: [], sinks: [] }, TITANIUM: { sources: [], sinks: [] }, CRYSTAL: { sources: [], sinks: [] }, UMBRITE: { sources: [], sinks: [] }, SHARD: { sources: [], sinks: [] } },
            techIds: [],
            domainIds: [],
            isAi: false,
            capitalTile: { x: 0, y: 0 }
      ...
      initialState: {
        tiles: [
          ...
          { x: 1, y: 1, terrain: "LAND", ownerId: "ai-1", ownershipState: "SETTLED", town: { type: "MARKET", populationTier: "TOWN", name: "Enemy" }, resource: "TITANIUM" },
    ...
    const message = await buildInitMessage(
      ...
      store,
      {
        ...
        player: {
          ...
          incomePerMinute: 1.2,
          strategicResources: { FOOD: 0, TITANIUM: 0, CRYSTAL: 0, UMBRITE: 0, SHARD: 0 },
          strategicProductionPerMinute: { FOOD: 2, TITANIUM: 0, CRYSTAL: 0, UMBRITE: 0, SHARD: 0 },
          developmentProcessLimit: 3,
