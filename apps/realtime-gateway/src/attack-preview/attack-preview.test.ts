...
describe("attackPreviewResult", () => {
  ...
  it("applies the target's active fort defense bonus", () => {
    ...
    expect(withFort.defMult).toBeCloseTo(2.5, 6);
    expect(withoutFort.defMult).toBeCloseTo(1, 6);
  ...
  it("ignores a fort that is not active or not owned by the defender", () => {
    ...
    expect(inactivePreview.defMult).toBeCloseTo(1, 6);
    expect(wrongOwnerPreview.defMult).toBeCloseTo(1, 6);
  });
  ...
  it("doubles the defender's defMult when the attacker has no war industry, and clears once both factory types exist", () => {
    ...
    expect(noWarIndustry.defMult).toBeCloseTo(2.0, 6);
    expect(bothFactoryTypes.defMult).toBeCloseTo(1, 6);
  });
});
