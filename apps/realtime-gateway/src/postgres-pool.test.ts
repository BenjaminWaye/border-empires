...
describe("createResilientPostgresPool", () => {
  ...
  it("registers an error listener so transient pool failures do not crash the gateway", async () => {
    const consoleError = vi.spyOn(console, "error").mockImplementation(() => undefined);
    const { createResilientPostgresPool } = await import("./postgres-pool.js");

    const pool = createResilientPostgresPool("postgres://localhost/test", "gateway-store");
    ...
    expect(consoleError).toHaveBeenCalledWith("[gateway-store] postgres pool error", error);
  ...
  it("reuses the same pool for the same connection string", async () => {
    ...
    const first = createResilientPostgresPool("postgres://localhost/shared", "gateway-store");
    const second = createResilientPostgresPool("postgres://localhost/shared", "gateway-recovery");
  ...
  it("disables TLS verification for Supabase-hosted connections", async () => {
    ...
    createResilientPostgresPool("postgresql://postgres:pw@db.example.supabase.co:5432/postgres", "gateway-store");
