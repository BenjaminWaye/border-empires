...
// createSeasonSeedWorld is synchronous and used where a Promise can't be
// awaited (e.g. inside the SimulationRuntime constructor, via seed-state.ts's
// "season-20ai" dev/test profile). createSeasonSeedWorldAsync below is the
// same generation but with cooperative yields between stages — used by the
// live "Start New Season" path (season-worldgen.ts), which runs on an armed
// watchdog alongside other traffic and must not monopolize the event loop.
// Keep the two loop bodies in sync when editing the generation sequence.
export const createSeasonSeedWorld = (
  seed: number,
  createPlayer: (id: string, isAi: boolean) => DomainPlayer,
  options: SeasonSeedWorldOptions = {}
...
export const createSeasonSeedWorldAsync = async (
  ...
  const yieldToLoop = async (): Promise<void> => {
  ...
  const tiles = new Map<string, DomainTileState>();
  for (let y = 0; y < WORLD_HEIGHT; y += 1) {
    if (y > 0 && y % 50 === 0) await yieldToLoop();
    for (let x = 0; x < WORLD_WIDTH; x += 1) {
      const tk = tileKey(x, y);
      const clusterId = clusterByTile.get(tk);
      const cluster = clusterId ? clustersById.get(clusterId) : undefined;
