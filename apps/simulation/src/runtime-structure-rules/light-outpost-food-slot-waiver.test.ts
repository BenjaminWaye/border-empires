...
// LIGHT_OUTPOST_FREE_FOOD_SLOT_COUNT (5): a player's first 5 Light Outposts
...
describe("BUILD_STRUCTURE parity — LIGHT_OUTPOST FOOD-slot waiver", () => {
  it("builds the first 5 Light Outposts with zero FOOD supply anywhere in the empire, rejects the 6th", async () => {
    vi.useFakeTimers();
    try {
      ...
      // Complete each build (advancing past LIGHT_OUTPOST_BUILD_MS) before
      ...
      for (let i = 0; i < 6; i++) {
        runtime.submitCommand({
          ...
          payloadJson: JSON.stringify({ x: 10 + i, y: 10, structureType: "LIGHT_OUTPOST" }),
        });
        await Promise.resolve();
        vi.advanceTimersByTime(structureBuildDurationMs("LIGHT_OUTPOST"));
        await Promise.resolve();
      }
      ...
      for (let i = 0; i < 5; i++) {
        expect(state.tiles.find((t) => t.x === 10 + i && t.y === 10)?.economicStructureJson).toContain('"type":"LIGHT_OUTPOST"');
      }
