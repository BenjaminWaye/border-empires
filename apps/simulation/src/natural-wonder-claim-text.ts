...
// Flavor + boon copy for the event-log entry fired when a player claims a
// natural wonder tile (runtime-natural-wonders.ts's announceNaturalWonderClaim).
// One line each, matching docs/natural-wonders-design.md §2's descriptions.
const NATURAL_WONDER_CLAIM_TEXT: Record<NaturalWonderType, { name: string; flavor: string; boon: string }> = {
  FOUNDRY_HEART: {
...
export const naturalWonderClaimEventText = (type: NaturalWonderType): string => {
  const { name, flavor, boon } = NATURAL_WONDER_CLAIM_TEXT[type];
