...
>(
  ...
  for (const delta of tileDeltas) {
    ...
    if (!visible) {
      ...
      // Note: buildSparseDelta now ALWAYS includes ownerId/ownershipState on
      // every emitted delta (see tile-delta-stringify-cache.ts), so this
      // check fires for any non-visible tile currently without an owner --
      // not only genuine clear transitions. That's fine for the ownerId
      // signal itself (it always matches ground truth), but we must NOT
      // forward the rest of the delta's substructure fields (fort/muster/
      // sabotage/yield/etc.) for a tile the player can't see -- only the
      // minimal ownership-state fields.
