...
export const findEnclosedRegion = (
  ...
  while (head < queue.length) {
    ...
    for (const [dx, dy] of DIRECTIONS) {
      ...
      // own SETTLED tiles wall the region in. A missing tile is treated as a
      // solid wall — the world map is dense, so an absent in-bounds key cannot
      // leak the region.
      if (!neighbor || neighbor.terrain !== "LAND" || (neighbor.ownerId === enclosingOwnerId && neighbor.ownershipState === "SETTLED")) continue;
      // Any other owner — including the enclosing player's own FRONTIER tiles,
      // which can still decay back to unowned — means the pocket isn't yet
      // walled by permanent territory, so it is left for manual play rather
      // than claimed.
