...
import { IRON_LEVY_MANPOWER_CONVERSION_RATIO, IRON_LEVY_REGEN_FREEZE_MS } from "@border-empires/game-domain";
import { parseIronLevyMusterPayload } from "./runtime-command-parsers.js";
...
export const IRON_LEVY_REGEN_FREEZE_KEY = "iron_levy_regen_freeze";

// The Iron Levy monument: converts IRON_LEVY_MANPOWER_CONVERSION_RATIO
...
// freezes the caster's empire-wide manpower regen for IRON_LEVY_REGEN_FREEZE_MS.
//
...
export function handleIronLevyMusterCommand(context: RuntimeMapCommandContext, command: CommandEnvelope): void {
  const actor = context.players.get(command.playerId);
  const payload = parseIronLevyMusterPayload(command.payloadJson);
  if (!actor || !payload) {
  ...
  if (
    ...
    anchor.economicStructure.type !== "IRON_LEVY" ||
  ...
  if (context.getAbilityCooldownUntil(actor.id, IRON_LEVY_REGEN_FREEZE_KEY) > now) {
    rejectCommand(context, command, "IRON_LEVY_MUSTER_INVALID", "manpower regen is still frozen from the last levy");
    return;
  }
  const convertedManpower = Math.floor(Math.max(0, actor.manpower) * IRON_LEVY_MANPOWER_CONVERSION_RATIO);
  if (convertedManpower <= 0) {
    rejectCommand(context, command, "IRON_LEVY_MUSTER_INVALID", "not enough banked manpower to levy");
    return;
  }
  ...
  context.setAbilityCooldownUntil(actor.id, IRON_LEVY_REGEN_FREEZE_KEY, now + IRON_LEVY_REGEN_FREEZE_MS);
  context.emitEvent({
