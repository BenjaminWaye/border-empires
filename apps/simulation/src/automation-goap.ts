...
export type AutomationGoapActionKey =
...
export type AutomationGoapDecision = {
  goalId: string;
  actionKey: AutomationGoapActionKey;
};

export const chooseAutomationGoapDecision = (
  state: AiEmpireGoapState,
  victoryPath?: AiSeasonVictoryPathId
): AutomationGoapDecision | undefined => {
  const plan = planBestGoal(state, goalsForVictoryPath(victoryPath), AI_EMPIRE_ACTIONS);
  const actionKey = plan?.steps[0]?.action.key as AutomationGoapActionKey | undefined;
  return plan && actionKey ? { goalId: plan.goalId, actionKey } : undefined;
};
