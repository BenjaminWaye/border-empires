...
import { sweepAttackCandidates } from "../territory-automation/territory-automation.js";
...
describe("sweep budget mechanics", () => {
  ...
  it("A2/A3: budget regens at player MP-regen rate and caps at SWEEP_BUDGET_CAP", async () => {
    ...
    await runtime.tickTerritoryAutomation(nowMs.value);
    ...
    await runtime2.tickTerritoryAutomation(nowMs2.value);
    const outpost2 = tileSiegeOutpost(runtime2, 10, 10);
    expect(outpost2?.sweepBudget).toBe(SWEEP_BUDGET_CAP);
  });

  it("A4: attack drains sweepBudget by SWEEP_ATTACK_COST", async () => {
    const nowMs = { value: 1_000 };
    const runtime = mkRuntime(
      [
        { x: 10, y: 10, terrain: "LAND", ownerId: "player-1", ownershipState: "SETTLED",
  ...
  it("A5: sweepBudget < SWEEP_ATTACK_COST → pause, sweepActive stays true, no attack", async () => {
    const nowMs = { value: 1_000 };
    const events: SimulationEvent[] = [];
    const runtime = mkRuntime(
      [
  ...
  it("A7: auto-resume after budget refills past threshold", async () => {
    const nowMs = { value: 1_000 };
    const events: SimulationEvent[] = [];
    const runtime = mkRuntime(
      [
...
describe("sweep targeting (outpost at 10,10, radius 5)", () => {
  it("B1: enemy tiles within radius included; outside excluded", async () => {
  ...
  it("B2/B3: closest target attacked first; deterministic tie-break (lower x then lower y)", async () => {
    const outpost = { x: 10, y: 10, terrain: "LAND" as const, ownerId: "player-1", ownershipState: "SETTLED" as const };
  ...
  it("B4: own tiles are not targeted", async () => {
    const outpost = { x: 10, y: 10, terrain: "LAND" as const, ownerId: "player-1", ownershipState: "SETTLED" as const };
    const tiles = new Map([
  ...
  it("B5: barbarian tiles included as valid targets", async () => {
    const outpost = { x: 10, y: 10, terrain: "LAND" as const, ownerId: "player-1", ownershipState: "SETTLED" as const };
    const tiles = new Map([
  ...
  it("B6: no targets in radius → sweepActive set to false", async () => {
    const nowMs = { value: 1_000 };
    const runtime = mkRuntime(
      [
        { x: 10, y: 10, terrain: "LAND", ownerId: "player-1", ownershipState: "SETTLED",
  ...
  it("B6b: after auto-deactivation, enemies returning to radius does NOT reactivate", async () => {
  ...
  it("B7: after capturing closest tile, next tick recomputes target", async () => {
    vi.useFakeTimers();
    try {
      const now = { value: 1_000 };
      const runtime = new SimulationRuntime({
      ...
      await runtime.tickTerritoryAutomation(now.value);
...
describe("sweep determinism and snapshot roundtrip", () => {
  it("I1: snapshot exportState preserves sweepBudget and sweepActive", async () => {
    const runtime = mkRuntime([
      { x: 10, y: 10, terrain: "LAND", ownerId: "player-1", ownershipState: "SETTLED",
  ...
  it("I2: same seed + commands produce same sweep targets (deterministic)", async () => {
    ...
    await r1.tickTerritoryAutomation(1_000);
    await r2.tickTerritoryAutomation(1_000);
...
describe("pause vs deactivate conflict", () => {
  it("budget tom AND no targets → deactivate wins (sweepActive = false)", async () => {
    const nowMs = { value: 1_000 };
    const runtime = mkRuntime(
      [
        { x: 10, y: 10, terrain: "LAND", ownerId: "player-1", ownershipState: "SETTLED",
...
describe("sweep attack uses bordering owned tile as origin", () => {
  it("attacks from a bordering owned neighbour (not the outpost) when target borders territory", async () => {
    // Layout:
  ...
  it("attacks a farther bordering candidate rather than expanding toward a closer non-bordering one", async () => {
...
describe("sweep expands toward target when it does not border owned territory", () => {
  it("issues EXPAND toward the target when target is out of reach", async () => {
    // Layout:
    //   (10,10) SETTLED outpost (player-1)
