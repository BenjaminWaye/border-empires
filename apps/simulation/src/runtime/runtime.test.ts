...
describe("simulation runtime", () => {
      ...
      new SimulationRuntime({
        ...
        initialState: {
          tiles: [
            ...
            // Gives the defender at least one Iron and one Fur Weapons
            ...
            // this test is about outpost aura, not that mechanic.
            {
              ...
              economicStructure: { ownerId: "player-2", type: "IRON_WEAPONS_FACTORY" as const, status: "active" as const }
            },
            {
              x: 8,
              y: 11,
              ...
              ownerId: "player-2",
              ownershipState: "SETTLED" as const,
              economicStructure: { ownerId: "player-2", type: "FUR_WEAPONS_FACTORY" as const, status: "active" as const }
            },
            ...(withOutpost
              ? [
                  {
  ...
  it("threads owned Weapons Workshop count into resolved combat atkEff and defEff", async () => {
    ...
    const buildRuntime = (attackerWorkshops: number, defenderWorkshops: number): SimulationRuntime => {
      ...
      const tiles = [
        ...
        // Gives the defender at least one Iron and one Fur Weapons Factory
  ...
  it("threads Iron/Fur Weapons Factory counts into resolved combat atkEff and defEff, scoped to each side's own connected-town network", async () => {
    ...
    const factoryTile = (x: number, y: number, ownerId: string, type: "IRON_WEAPONS_FACTORY" | "FUR_WEAPONS_FACTORY") => ({
      x,
      y,
      terrain: "LAND" as const,
      ownerId,
    ...
    const runtime = new SimulationRuntime({
      ...
      initialState: {
        tiles: [
          ...
          // attack itself. 2 Iron + 1 Fur Weapons Factory on its support tiles.
          { x: 50, y: 50, terrain: "LAND", ownerId: "player-1", ownershipState: "SETTLED", town: { name: "Capital", type: "FARMING", populationTier: "TOWN" } },
          factoryTile(50, 49, "player-1", "IRON_WEAPONS_FACTORY"),
          factoryTile(49, 50, "player-1", "IRON_WEAPONS_FACTORY"),
          factoryTile(51, 50, "player-1", "FUR_WEAPONS_FACTORY"),
          // Resource-slot backing (§5.4) so the three factories above aren't
          // dormant: 2 IRON slots for the 2 Iron Weapons Factories, 1 SUPPLY
          // slot (via WOOD) for the 1 Fur Weapons Factory.
          { x: 52, y: 50, terrain: "LAND", resource: "IRON" as const, ownerId: "player-1", ownershipState: "SETTLED" as const },
          { x: 53, y: 50, terrain: "LAND", resource: "IRON" as const, ownerId: "player-1", ownershipState: "SETTLED" as const },
          { x: 54, y: 50, terrain: "LAND", resource: "WOOD" as const, ownerId: "player-1", ownershipState: "SETTLED" as const },
          // Defender's own network: 1 Iron + 1 Fur Weapons Factory adjacent
          // to the target town itself, which also neutralizes the "unarmed"
          // vulnerability multiplier (both types present).
          factoryTile(9, 11, "player-2", "IRON_WEAPONS_FACTORY"),
          factoryTile(9, 12, "player-2", "FUR_WEAPONS_FACTORY"),
          { x: 8, y: 11, terrain: "LAND" as const, resource: "IRON" as const, ownerId: "player-2", ownershipState: "SETTLED" as const },
          { x: 7, y: 11, terrain: "LAND" as const, resource: "WOOD" as const, ownerId: "player-2", ownershipState: "SETTLED" as const }
        ],
        activeLocks: []
      }
    });
  ...
  it("doubles attacker effectiveness when the defender has no war industry, and clears once both factory types exist", async () => {
    ...
    const buildRuntime = (defenderFactories: Array<"IRON_WEAPONS_FACTORY" | "FUR_WEAPONS_FACTORY">): SimulationRuntime =>
      new SimulationRuntime({
        now: () => 1_000,
        initialPlayers: new Map([
          ["player-1", buildPlayer("player-1", { manpower: 5_000 })],
    ...
    const ironOnly = await captureAtkEff(buildRuntime(["IRON_WEAPONS_FACTORY"]));
    const both = await captureAtkEff(buildRuntime(["IRON_WEAPONS_FACTORY", "FUR_WEAPONS_FACTORY"]));

    // Missing both -> flat 2x, same as missing just one (confirmed design:
    // does not stack to 4x).
    expect(neitherFactory).toBeCloseTo(20, 6);
    expect(ironOnly).toBeCloseTo(20, 6);
