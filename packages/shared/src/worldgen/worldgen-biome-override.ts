...
import { setBiomeCacheEntry, worldIndex } from "./worldgen.js";
...
// over the 500-line file cap; setBiomeCacheEntry stays in worldgen.ts itself
// so the encode+ready-flag invariant can't be broken from out here.
export const overrideLandBiomeAt = (x: number, y: number, biome: LandBiome): void => {
  setBiomeCacheEntry(worldIndex(wrapX(x, WORLD_WIDTH), wrapY(y, WORLD_HEIGHT)), biome);
};
