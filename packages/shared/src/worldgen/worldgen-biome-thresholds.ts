...
//
// v3 fixes the actual cause: sandField/forestField/hillField now blend in a
// small-cell (2-3 tile) "mottle" octave at high weight (see
// mottledFieldAt below), so the field itself flips on a per-few-tiles
// basis like Civilization-style terrain, with a low-weight large-cell
// "climate" octave riding along so the overall region still reads as
// more/less arid or forested rather than pure static. Thresholds are
// recalibrated again against this new field's distribution (measured via a
// scanline run-length harness against a fixed seed).
//
...
export const sandFieldAt = (wx: number, wy: number, seed: number, version: number): number =>
  version < 3
    ...
    : mottledField(valueNoise(wx, wy, 60, seed + 303), valueNoise(wx - 41, wy + 29, 2, seed + 317), 0.15);
...
export const forestFieldAt = (wx: number, wy: number, seed: number, version: number): number =>
  version < 3
    ...
    : mottledField(valueNoise(wx + 41, wy - 23, 60, seed + 99), valueNoise(wx - 17, wy + 61, 2, seed + 109), 0.15);
...
export const hillFieldAt = (wx: number, wy: number, seed: number, version: number): number =>
  version < 3
    ...
    : mottledField(valueNoise(wx + 211, wy - 97, 60, seed + 811), valueNoise(wx - 53, wy + 137, 3, seed + 821), 0.2);
