...
// isolated factory on a neutral backdrop still looks firmly anchored instead
...
const render = (args: Args, groundRadius: number): HTMLElement => {
  ...
  const overlay = createUmbriteWeaponsFactoryOverlay(stage.scene, Math.max(args.count, 1));
...
const meta: Meta<Args> = {
  title: "3D Library/UmbriteWeaponsFactory",
  argTypes: {
...
// The hero asset shot: a single factory, isolated on a light neutral studio
// backdrop so the dark iron, brass piping and contained ember reactor energy
...
export const FactoryHero: Story = {
  render: () => {
...
// Same single-factory hero shot on a dark neutral backdrop — how the complex
// reads on the game's night/dark terrain and in dark UI panels.
export const FactoryHeroDark: Story = {
  render: () => {
...
// A single factory in normal gameplay context — perspective camera, grass
// ground — to check how the hall, reactor and ammo rack hold up at playing
// distance rather than studio close-up.
export const OnGrass: Story = {
  render: () => render({ cameraDistance: 5, spacing: 1.4, count: 1 }, 3)
};

// A small factory district — how a cluster of factory tiles reads from the
// normal game camera.
...
// Side-by-side: the dark military factory next to a silver-grey titanium
// deposit, so the Umbrite complex's silhouette and scale can be compared
// against an existing resource asset.
export const FactoryVsTitanium: Story = {
  render: () => {
    const stage = glintStage({ cameraDistance: 6.5 });
    const ground = createGrassGround(2, 0);
    stage.scene.add(ground.group);
    const factory = createUmbriteWeaponsFactoryOverlay(stage.scene, 3);
    const titanium = createTitaniumDepositOverlay(stage.scene, 3);
    for (let i = 0; i < 3; i += 1) {
      const x = (i - 1) * 1.4;
      factory.addInstance(x - 1.0, 0, 0, i, 0);
