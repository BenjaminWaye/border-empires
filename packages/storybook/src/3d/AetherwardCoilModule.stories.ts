...
// horseshoe coil faces +X (the socket yaw). Rotating the content container by
// -90° plus an optional display yaw brings the coil's open mouth toward the
// front of frame.
const buildSingleModule = (
  ...
  const overlay = createAetherwardCoilModuleOverlay(inner, 1);
...
const meta: Meta<Args> = {
  title: "3D Library/AetherwardCoilModule",
  argTypes: {
...
// The hero asset shot: the aether-field tuning cartridge docked and aimed at
// the camera on a light neutral studio backdrop — the oversized upright
// horseshoe coil with the suspended cyan field node cradled in its open mouth.
export const AetherwardCoilHero: Story = {
  render: () => {
...
export const AetherwardCoilHeroDark: Story = {
  render: () => {
...
// docked seat, pod, the horseshoe coil and the rear coupling.
...
export const DockedToAfc: Story = {
  ...
  render: (args) => {
    ...
    afc.commit();
    const modules = createAetherwardCoilModuleOverlay(stage.scene, 1);
    // Socket at the 90° azimuth faces the default camera — the module reads
    // coil-first with the empty bays flanking it.
