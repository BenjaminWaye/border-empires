...
// resonance core faces +X (the socket yaw). Rotating the content container by
// -90° plus an optional display yaw brings the core toward the front of frame.
const buildSingleModule = (
  ...
  const overlay = createAetherResonanceModuleOverlay(inner, 1);
...
const meta: Meta<Args> = {
  title: "3D Library/AetherResonanceCore",
  argTypes: {
...
// The hero asset shot: the aether-calibration cartridge docked and aimed at
// the camera on a light neutral studio backdrop — the glowing cyan resonance
// core inside its brass frame, wrapped by the oversized induction coils.
export const AetherResonanceHero: Story = {
  render: () => {
...
export const AetherResonanceHeroDark: Story = {
  render: () => {
...
// docked seat, pod, core frame with the fanned coils and the rear coupling.
...
export const DockedToAfc: Story = {
  ...
  render: (args) => {
    ...
    afc.commit();
    const modules = createAetherResonanceModuleOverlay(stage.scene, 1);
    // Socket at the 90° azimuth faces the default camera — the module reads
    // core-first with the empty bays flanking it.
