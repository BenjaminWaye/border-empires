...
// The default Storybook camera looks down the +Z axis, and the module's drill
// head faces +X (the socket yaw). Rotating the content container by -90° plus
// an optional display yaw brings the drill toward the front of the frame.
const buildSingleModule = (
  ...
  const overlay = createRiggingWorksModuleOverlay(inner, 1);
...
// The hero asset shot: the drill-head fabrication cartridge docked and aimed
// at the camera on a light neutral studio backdrop — oversized auger, chunky
// gearbox with brass drive rings and the flank cable drum front and center.
export const RiggingWorksHero: Story = {
  render: () => {
...
export const RiggingWorksHeroDark: Story = {
  render: () => {
...
// docked seat, pod, drill barrel with pistons, gearbox rings and the drum.
...
export const DockedToAfc: Story = {
  ...
  render: (args) => {
    ...
    afc.commit();
    const modules = createRiggingWorksModuleOverlay(stage.scene, 1);
    // Socket at the 90° azimuth faces the default camera — the module reads
    // drill-first with the empty bays flanking it.
