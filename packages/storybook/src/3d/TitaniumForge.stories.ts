...
// The default Storybook camera looks down the +Z axis, and the module's forge
// mouth faces +X (the socket yaw). Rotating the content container by -90° plus
// an optional display yaw brings the mouth toward the front of the frame.
const buildSingleModule = (
  ...
  const overlay = createTitaniumForgeModuleOverlay(inner, 1);
...
const meta: Meta<Args> = {
  title: "3D Library/TitaniumForge",
  argTypes: {
...
// neutral studio backdrop — the large glowing forge mouth and chunky press
// front and center, rounded pod and side braces in support.
export const TitaniumForgeHero: Story = {
  render: () => {
...
export const TitaniumForgeHeroDark: Story = {
  render: () => {
...
// docked seat, rounded pod, forge mouth with ribs and the press ram.
...
export const DockedToAfc: Story = {
  ...
  render: (args) => {
    ...
    afc.commit();
    const modules = createTitaniumForgeModuleOverlay(stage.scene, 1);
    // Socket at the 90° azimuth faces the default camera — the module reads
    // forge-first with the empty bays flanking it.
