...
const buildSingleModule = (
  ...
  const overlay = createStratosphericDockyardModuleOverlay(inner, 1);
...
const meta: Meta<Args> = {
  title: "3D Library/StratosphericDockyardModule",
  argTypes: {
...
// The hero asset shot: the airborne-assembly rig docked and aimed at the
// camera on a light neutral studio backdrop — the wide open U-shaped docking
// cradle over its low pod, with the short rear lifting mast behind it.
export const StratosphericDockyardHero: Story = {
  render: () => {
...
export const StratosphericDockyardHeroDark: Story = {
  render: () => {
...
// docked seat, low pod, the open cradle and its empty bay, the rear mast with
// its capstan winch, the hoist cables, and the rear coupling.
...
// it actually reads when seated in a socket on the map, cradle open to the
// sky.
...
export const DockedToAfc: Story = {
  ...
  render: (args) => {
    ...
    afc.commit();
    const modules = createStratosphericDockyardModuleOverlay(stage.scene, 1);
    // Socket at the 90° azimuth faces the default camera — the open cradle
    // reads front-on with the empty bays flanking it.
