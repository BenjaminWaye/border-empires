...
import { createUpgradeReadyBadgeOverlay } from "@client/client-map-3d-upgrade-ready-badge-overlay/client-map-3d-upgrade-ready-badge-overlay.js";
import { createStage, forEachGridCell, wrapWithCleanup } from "../three-stage.js";

// The green up-arrow badge that floats over an owned, settled town that has
// grown enough population to upgrade to its next tier (see
// client-map-3d-upgrade-ready-badge-overlay.ts). Drives the overlay's tick()
// on its own RAF so the bob animation plays in the story — the shared stage
// only renders, it doesn't advance overlays.
...
const render = (args: Args): HTMLElement => {
  ...
  const overlay = createUpgradeReadyBadgeOverlay(stage.scene, maxTiles);
...
const meta: Meta<Args> = {
  title: "3D Library/UpgradeReadyBadgeOverlay",
  argTypes: {
