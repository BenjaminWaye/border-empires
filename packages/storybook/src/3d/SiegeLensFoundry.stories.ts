...
import { createSiegeLensFoundryModuleOverlay } from "@client/client-map-3d-siege-lens-foundry-module.js";
...
// The default Storybook camera looks down the +Z axis, and the module's lens
// faces +X (the socket yaw). Rotating the content container by -90° plus an
// optional display yaw brings the lens toward the front of the frame.
const buildSingleModule = (
  ...
  const overlay = createSiegeLensFoundryModuleOverlay(inner, 1);
...
const meta: Meta<Args> = {
  title: "3D Library/SiegeLensFoundry",
  argTypes: {
...
// The hero asset shot: the module docked and aimed at the camera on a light
// neutral studio backdrop — the large brass-and-steel lens assembly front
// and center, foundry body and flanks in support.
export const SiegeLensFoundryHero: Story = {
  render: () => {
...
export const SiegeLensFoundryHeroDark: Story = {
  render: () => {
...
// docked seat, hexagonal foundry, tanks, clamps and lens aperture.
...
export const DockedToAfc: Story = {
  ...
  render: (args) => {
    ...
    afc.commit();
    const modules = createSiegeLensFoundryModuleOverlay(stage.scene, 1);
    // Socket at the 90° azimuth faces the default camera — the module reads
    // lens-first with the empty bays flanking it.
