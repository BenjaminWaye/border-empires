...
describe("login and frontier retry regression guard", () => {
  ...
  it("clears queued frontier retries once combat or tile updates confirm the tile is now yours", () => {
    ...
    const timersSource = clientSource("./client-runtime-timers.ts");
    expect(networkSource).toContain('state.frontierSyncWaitUntilByTarget.delete(keyFor(mergedTile.x, mergedTile.y));');
    expect(timersSource).toContain('state.actionQueue = state.actionQueue.filter((entry) => deps.keyFor(entry.x, entry.y) !== currentKey);');
    expect(timersSource).toContain('state.actionQueue = state.actionQueue.filter((entry) => deps.keyFor(entry.x, entry.y) !== timedOutCurrentKey);');
  });
