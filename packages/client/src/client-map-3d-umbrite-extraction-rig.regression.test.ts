...
import { createUmbriteExtractionRigOverlay } from "./client-map-3d-umbrite-extraction-rig.js";
...
describe("umbrite extraction rig overlay", () => {
  it("commits a fully assembled rig with visible pieces", () => {
    const scene = new Scene();
    const overlay = createUmbriteExtractionRigOverlay(scene, 3);
  ...
  it("clears instance counts without removing meshes from the scene", () => {
    const scene = new Scene();
    const overlay = createUmbriteExtractionRigOverlay(scene, 1);
  ...
  it("does not allocate instance capacity proportional to per-tile piece count", () => {
    // The rig emits 54 pieces per tile (deck, column, drill, pipes, tank,
    // vein lumps and energy bits); caps are sized to that worst case and
    // stay far under a 64MB budget even for a fully-rigged viewport.
    const scene = new Scene();
    const overlay = createUmbriteExtractionRigOverlay(scene, 14_000);
