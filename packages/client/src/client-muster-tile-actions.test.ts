...
describe("buildMusterActions", () => {
  it("returns nothing for a tile the player doesn't own", () => {
    stubWindowStorage();
    expect(buildMusterActions(ownTile({ ownerId: "rival" }), { me: "me", authEmail: "", manpower: 0, manpowerCap: MANPOWER_CAP })).toEqual([]);
  });

  it("hides the muster option on an un-mustered tile before the player has met a rival empire", () => {
    stubWindowStorage();
    expect(buildMusterActions(ownTile(), { me: "me", authEmail: "a@example.com", manpower: 0, manpowerCap: MANPOWER_CAP })).toEqual([]);
  });
  ...
  it("offers Stage Muster once mustering has been unlocked", async () => {
    ...
    const actions = buildMusterActions(ownTile(), { me: "me", authEmail: "a@example.com", manpower: 0, manpowerCap: MANPOWER_CAP });
  ...
  it("still shows Clear Muster for an existing flag even if never explicitly unlocked", () => {
    ...
    const actions = buildMusterActions(
      ...
      { me: "me", authEmail: "a@example.com", manpower: MUSTER_FLAG_CAP_UPGRADE_COST, manpowerCap: MANPOWER_CAP }
    );
  ...
  it("disables Expand Capacity when the player can't afford it", () => {
    ...
    const actions = buildMusterActions(
      ownTile({ muster: { ownerId: "me", amount: 40, mode: "HOLD", updatedAt: 0 } }),
      { me: "me", authEmail: "a@example.com", manpower: MUSTER_FLAG_CAP_UPGRADE_COST - 1, manpowerCap: MANPOWER_CAP }
    );
  ...
  it("clamps the default cap at MUSTER_FLAG_BASE_CAP_CEILING once 10% would exceed it", () => {
    ...
    const actions = buildMusterActions(
      ownTile({ muster: { ownerId: "me", amount: 40, mode: "HOLD", updatedAt: 0 } }),
      { me: "me", authEmail: "a@example.com", manpower: MUSTER_FLAG_CAP_UPGRADE_COST, manpowerCap: 10_000 }
    );
