 ...
 * anyone but the player -- a rival empire or barbarians, either is something
 * worth mustering against -- and queues the "First Contact" tip. See
 * client-muster-tile-actions.ts for where the unlock flag is read to gate
 * the muster tile actions. No-ops once already unlocked; this only ever
 * flips one way.
 */
export const unlockMusterOnEnemyContact = (
  ...
  if (!seenTile?.ownerId || seenTile.ownerId === me) return;
