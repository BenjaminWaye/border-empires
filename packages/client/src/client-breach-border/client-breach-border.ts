...
/**
 * Draws a jagged, torn-paper-style border overlay on a tile that is
 * currently inside its Breakthrough Momentum breach window (see
 * BREAKTHROUGH_DEBUFF_MULT / BREAKTHROUGH_DURATION_MS in
 * @border-empires/shared) — a visual cue that the tile's defence is
 * temporarily weakened after a neighbouring tile was just captured.
 */
export const drawBreachTornBorder = (
  ...
  ctx.beginPath();
  traceTornEdge(ctx, x1, y1, x2, y1, tile.x, tile.y, 0, depth);
  traceTornEdge(ctx, x2, y1, x2, y2, tile.x, tile.y, 1, depth);
  traceTornEdge(ctx, x2, y2, x1, y2, tile.x, tile.y, 2, depth);
  traceTornEdge(ctx, x1, y2, x1, y1, tile.x, tile.y, 3, depth);
  ctx.stroke();
  ctx.restore();
};
