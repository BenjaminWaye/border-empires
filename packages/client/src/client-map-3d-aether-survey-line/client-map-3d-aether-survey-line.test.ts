...
import { lerpPoint, pulseT, tileQuadCorners } from "./client-map-3d-aether-survey-line.js";
...
describe("pulseT", () => {
  it("loops into [0, 1)", () => {
    expect(pulseT(0, 1000)).toBe(0);
    expect(pulseT(500, 1000)).toBe(0.5);
    expect(pulseT(1000, 1000)).toBe(0);
    expect(pulseT(1500, 1000)).toBe(0.5);
  });

  it("handles negative time (wraps forward instead of going negative)", () => {
    const t = pulseT(-250, 1000);
    expect(t).toBeGreaterThanOrEqual(0);
    expect(t).toBeLessThan(1);
    expect(t).toBeCloseTo(0.75);
  });
});
