...
import {
  ...
  type CornerCoord,
  type TileCoord
} from "./client-reach-overlay-boundary-trace.js";
...
describe("traceReachBoundaryEdgeLoops", () => {
  ...
  it("keeps a 2x2 block and a single tile that touch only at one diagonal corner as separate loops, instead of hopping across the pinch mid-walk", () => {
    // Block A (tiles (0,0),(1,0),(0,1),(1,1)) and tile B=(2,2) touch only at
    // grid corner (2,2) -- A's bottom-right corner is exactly B's top-left
    // corner, but no tile of A is edge-adjacent to B. That corner is a
    // "saddle": it has two unrelated outgoing edges (A's own continuation,
    // and B's), and A's natural boundary walk reaches this corner *mid-walk*
    // (it's A's 5th corner out of 8, not A's start/end).
    //
    // `region`'s insertion order is deliberately adversarial: A's own start
    // tile (0,0) is processed first (so A's walk begins there and reaches
    // the saddle corner before anything else touches it), but B is
    // processed *before* A's corner-owning tile (1,1) (so B's edge lands
    // first in that corner's candidate list). A tracer that resolves the
    // saddle by list order (instead of by which edge actually continues the
    // incoming walk direction) grabs B's edge for A's continuation here,
    // making the walk detour out onto B's whole perimeter and back --
    // visiting the saddle corner twice and producing one self-touching
    // "figure eight" loop instead of two separate ones. In a larger real
    // territory this same wrong turn is what stitches two distant pieces of
    // border into one loop with a bogus long chord between them, which
    // samplePerimeterPylons's distance guard then drops -- leaving two real
    // pylons with no segment connecting them (the reported border "gate").
    const region: TileCoord[] = [
      { x: 0, y: 0 },
      { x: 2, y: 2 }, // B, inserted before A's (1,1) so its edge lists first at the saddle
      { x: 1, y: 0 },
      { x: 0, y: 1 },
      { x: 1, y: 1 } // A's corner tile -- owns the saddle's other outgoing edge
    ];
    const deps = buildDeps(region);
    const reach = toReach(region);
    ...
    for (const loop of loops) {
      expect(consecutiveCornersAreUnitSteps(loop)).toBe(true);
      // No corner should be visited twice within a single loop -- a
      // self-touching "figure eight" (the bug) revisits the saddle corner.
      expect(new Set(loop.map((c) => keyFor(c.x, c.y))).size).toBe(loop.length);
    }
    const spans = loops.map((loop) => loop.length);
    spans.sort((a, b) => a - b);
    expect(spans).toEqual([4, 8]); // B's 1-tile loop, A's 2x2-block loop
  });
});
