...
//   │     wrapped by three large segmented induction-coil arcs that channel
//   │     energy toward the center. The core surges gently as it calibrates.
...
export const createAetherResonanceModuleOverlay = (scene: Scene, maxInstances: number, buildingEnvironmentTexture?: Texture): AetherResonanceModuleOverlay => {
  ...
  const TAU = Math.PI * 2;
  // Resonance surge — the cyan core advances slightly within its frame as it
  // calibrates (a gentle energy pulse, not a rotating machine part).
  const CORE_SPEED = 0.002;
  const CORE_PULSE = 0.03;
  ...
  // ─── Resonance idle pulse ───────────────────────────────────────────
  // The two cyan core discs surge gently along the forward axis (a calibration
  // pulse); the coils and frame stay rigid.
  const plateE = eulerFromDir(1, 0, 0);
  const plateQuat = new Quaternion().setFromEuler(new Euler(plateE.rx, plateE.ry, plateE.rz, "XYZ"));
  const coreGroup: Array<{ key: string; ox: number }> = [
    { key: "core", ox: 0.088 },
    { key: "coreInner", ox: 0.098 }
  ];
  ...
  const addInstance = (sceneX: number, sceneZ: number, surfaceY: number, yaw: number, worldTileX: number, worldTileY: number): number => {
    ...
    const phase = ((hash % 1000) / 1000) * TAU;
  ...
  const update = (nowMs: number): void => {
    ...
    for (let i = 0; i < count; i += 1) {
      ...
      yawMatrix.makeRotationFromQuaternion(yawQuat);
      const surge = CORE_PULSE * Math.sin(nowMs * CORE_SPEED + rec.phase);
      for (const piece of coreGroup) {
        const slot = slots.get(piece.key);
        if (!slot || slot.count <= i) continue;
        position.set(rec.x + (piece.ox + surge) * AETHER_CORE_SCALE, rec.y + 0.16 * AETHER_CORE_SCALE, rec.z);
        scale.set(AETHER_CORE_SCALE, AETHER_CORE_SCALE, AETHER_CORE_SCALE);
        pieceMatrix.compose(position, plateQuat, scale);
        matrix.multiplyMatrices(yawMatrix, pieceMatrix);
