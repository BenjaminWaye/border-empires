...
describe("settlement overlay regression guard", () => {
  ...
  it("does not treat settlements as support-building anchors or build hosts", () => {
    ...
    const roadNetworkSource = clientSource("./client-road-network.ts");
    const overviewModifierSource = clientSource("./client-tile-overview-modifiers.ts");
    ...
    expect(tileMenuSource).toContain('if (hasOwnedLandState && tile.town.populationTier !== "SETTLEMENT") {');
