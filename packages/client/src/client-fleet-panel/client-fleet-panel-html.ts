  ...
  return `
    <div class="fl-hull-card ${count > 0 ? "fl-hull-card-active" : ""}" data-fleet-hull-card="${id}">
      ...
      <div class="fl-hull-stats">
        <span title="Production cost">💰${info.cost}</span>
        <span title="Damage">💥${info.damage}</span>
        <span title="Relative speed">⚡${info.speed}</span>
      </div>
...
export const fleetOrderListHtml = (orders: FleetOrderView[]): string =>
  orders.length > 0
    ? `<ul class="fl-order-list">${orders
        .map((o) => {
          const icon = o.status === "TRAVELING" ? "🚀" : o.reconOnly ? "🔍" : "⚔️";
          return `
      <li class="fl-order fl-status-${o.status.toLowerCase()}">
        ...
        ${o.status === "TRAVELING" ? `<span class="fl-order-eta">arrives ${relativeTimeFromNow(o.arrivesAt)}</span>` : ""}
