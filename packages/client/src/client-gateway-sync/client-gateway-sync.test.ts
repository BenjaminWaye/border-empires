...
describe("client gateway sync", () => {
  ...
  it("clears a stale yieldRate/yieldCap when a delta removes the economicStructure without providing a fresh value (docs/plans/2026-07-06-radius-yield-delivery.md Phase 4)", () => {
    const deps = createDeps();
      ...
      tiles: [
        {
          ...
          ownershipState: "SETTLED",
          resource: "FARM",
          economicStructureJson: JSON.stringify({ ownerId: "me", type: "FARMSTEAD", status: "active" }),
    ...
    expect(deps.state.tiles.get("20,20")?.yieldRate?.strategicPerDay?.FOOD).toBe(162);

    // Structure destroyed — delta clears economicStructureJson but sends no fresh yieldRate/yieldCap.
    applyGatewayTileDeltaBatch(deps, [{ x: 20, y: 20, economicStructureJson: "" }]);

    const resolved = deps.state.tiles.get("20,20");
    // Re-derived locally: bare FARM tile with no structure is 72/day, not the stale 162.
    expect(resolved?.yieldRate?.strategicPerDay?.FOOD).toBe(72);
  });

  it("clears a stale yieldRate/yieldCap when a delta removes the dockId without providing a fresh value", () => {
    const deps = createDeps();
      ...
      tiles: [
        {
          ...
          yieldCap: { gold: 100, strategicEach: 24 }
        }
      ]
    });

    expect(deps.state.tiles.get("21,20")?.yieldRate?.goldPerMinute).toBe(5);

    // Dock removed — delta clears dockId but sends no fresh yieldRate/yieldCap.
    applyGatewayTileDeltaBatch(deps, [{ x: 21, y: 20, dockId: "" }]);

    const resolved = deps.state.tiles.get("21,20");
    // Re-derived locally: no dock, no town → no yield at all.
    expect(resolved?.yieldRate).toBeUndefined();
  });

  it("keeps the server-provided yieldRate when the same delta supplies a fresh value alongside the structure change", () => {
    const deps = createDeps();
      ...
      tiles: [
        {
          ...
          ownershipState: "SETTLED",
          resource: "IRON",
          economicStructureJson: JSON.stringify({ ownerId: "me", type: "MINE", status: "active" }),
    ...
    // Foundry boost applied server-side; delta carries both the structure change and the fresh rate.
    applyGatewayTileDeltaBatch(deps, [
      {
        x: 22,
        y: 20,
    ...
    expect(deps.state.tiles.get("22,20")?.yieldRate?.strategicPerDay?.IRON).toBe(180);
  });
