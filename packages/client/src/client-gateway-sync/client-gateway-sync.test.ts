...
const createDeps = (overrides?: { me?: string; mods?: { income?: number }; realMergeIncomingTileDetail?: boolean }) => {
  const state = {
      ...
      tilesRevision: 0, tilesRevisionChangedKeys: new Set<string>(), tilesRevisionOverflowed: false,
...
describe("client gateway sync", () => {
  ...
  // Regression: normalizeGatewayTileDelta's frontierDecayKind mapping only
  // recognized "ENCIRCLEMENT" and silently dropped any other value to
  // undefined -- including "OUT_OF_REACH", the sim's other real decay kind.
  // That meant a tile decaying for being outside reach synced its
  // frontierDecayAt deadline fine but lost frontierDecayKind on the way in,
  // so the tile menu's header status (tileMenuHeaderStatusForTile) could
  // never match the OUT_OF_REACH branch and fell back to a static "Outside
  // reach" with no live countdown.
  it("applies OUT_OF_REACH frontier decay timers from gateway tile deltas", () => {
    const deps = createDeps();
      ...
      tiles: [
        {
          ...
          ownerId: "me",
          ownershipState: "FRONTIER",
          frontierDecayAt: 701_000,
          frontierDecayKind: "OUT_OF_REACH"
        }
      ]
    });

    expect(deps.state.tiles.get("21,22")?.frontierDecayAt).toBe(701_000);
    expect(deps.state.tiles.get("21,22")?.frontierDecayKind).toBe("OUT_OF_REACH");
    ...
    expect(deps.state.tiles.get("21,22")?.frontierDecayAt).toBeUndefined();
    expect(deps.state.tiles.get("21,22")?.frontierDecayKind).toBeUndefined();
  });
