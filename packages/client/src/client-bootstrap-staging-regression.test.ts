...
describe("client bootstrap staging regression", () => {
  it("requests the first world view immediately at radius 0 and widens in stages after the first chunk", () => {
    const source = readFileSync(new URL("./client-network.ts", import.meta.url), "utf8");
    expect(source).toContain("requestViewRefresh(0, true);");
  ...
  it("keeps bootstrap retries at radius 0 while the first chunk is still pending", () => {
    ...
    expect(source).toContain("deps.requestViewRefresh(0, true);");
