...
export const menuOverviewForTile = (
  ...
  if (tile.muster) {
    const cap = 120;
    const staged = tile.muster.amount.toFixed(0);
    const mode = tile.muster.mode === "ADVANCE" ? "ADVANCE — will auto-attack when full" : "HOLD";
    pushLine(`Manpower staged: ${staged} / ${cap} · ${mode}`);
  }
