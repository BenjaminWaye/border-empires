...
// The Worldbreaker Cannon is a colossal monument assembled from parts
// dropped on support tiles. The wire protocol only carries a single
// type string ("WORLD_ENGINE_PART") for every part tile, so this family
// registers the wire kind plus three client-only variant kinds — one per
// component asset — and resolves which variant a tile shows
// deterministically from its coordinates. The wire kind renders the Long
// Barrel (the most iconic "cannon part" read).
...
export const WORLDBREAKER_PART_VARIANTS: ReadonlyArray<WorldbreakerPartStructureKind> = [
  "WORLD_ENGINE_PART_BARREL",
  "WORLD_ENGINE_PART_CORE",
  "WORLD_ENGINE_PART_ARRAY"
];

// Stable, coordinate-based variant pick so the parts of one monument
// settle on distinct assets without any wire changes. The double-mod
// keeps the index positive for negative world coords.
export const worldbreakerPartVariantForTile = (
  wx: number,
  wy: number
): WorldbreakerPartStructureKind => {
  const idx = ((wx * 7 + wy * 13) % WORLDBREAKER_PART_VARIANTS.length + WORLDBREAKER_PART_VARIANTS.length) % WORLDBREAKER_PART_VARIANTS.length;
  return WORLDBREAKER_PART_VARIANTS[idx]!;
};
...
export const registerWorldbreakerPartStructures = (
  ...
  return {
    layouts: {
      WORLD_ENGINE_PART: addLongBarrel,
      WORLD_ENGINE_PART_BARREL: addLongBarrel,
      WORLD_ENGINE_PART_CORE: addFractureCore,
      WORLD_ENGINE_PART_ARRAY: addSkyMarkingArray
    }
  };
};
