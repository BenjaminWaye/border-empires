...
// techCurrentModsHtml and its private helpers moved to
// client-tech-current-mods-html.ts (500-line source budget).
export { techCurrentModsHtml } from "./client-tech-current-mods-html.js";
...
export const effectSummaryLabel = (key: string, value: unknown): string | null => {
  ...
  if (key === "unlockAssemblyWorks" && value === true) return "Unlocks Neural Works";
  if (key === "unlockAncillaryDepot" && value === true) return "Unlocks Ancillary Depot";
  if (key === "unlockReserveLattice" && value === true) return "Unlocks Reserve Lattice";
