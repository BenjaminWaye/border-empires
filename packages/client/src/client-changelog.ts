...
export const LATEST_CLIENT_CHANGELOG: ClientChangelogRelease = {
  ...
  entries: [
    ...
    {
      introducedIn: "2026.05.04.2",
      title: "Reveal snapshot refreshes now expose memory and payload diagnostics",
      why: "Late-game and fog-toggle investigations needed harder numbers, because the legacy server could rebuild large reveal snapshots without showing how much memory, cache space, or websocket payload volume each refresh was actually consuming.",
      changes: [
        "Legacy chunk snapshot telemetry now records trigger type, world size, payload bytes, peak RSS and heap use, and per-player cache pressure for reveal-related refreshes.",
        "The runtime admin dashboard now breaks cached reveal data into payload, visibility-mask, and snapshot bytes, and highlights the heaviest per-player cache holders for faster staging diagnosis.",
        "Added server regression coverage for the new snapshot trigger plumbing, queued refresh replay labeling, and cache-byte accounting so future reveal-path changes keep emitting the same diagnostics."
      ]
    },
    {
