...
describe("createChunkSnapshotController", () => {
  ...
  it("defers chunk snapshot streaming while human frontier priority is active", async () => {
    vi.useFakeTimers();
  ...
  it("records trigger, payload bytes, and player cache bytes in perf samples", async () => {
    vi.useFakeTimers();
    const actor = makePlayer("player-1");
    const perfSamples: Array<Record<string, number | string>> = [];
    const controller = createChunkSnapshotController<Player>({
      chunkSize: 1,
      chunkCountX: 2,
      chunkCountY: 1,
      ...
      wrapChunkX: (value) => value,
      wrapChunkY: (value) => value,
      runtimeMemoryStats: () => ({ rssMb: 321, heapUsedMb: 111, heapTotalMb: 0, externalMb: 0, arrayBuffersMb: 0 }),
      pushChunkSnapshotPerf: (sample) => perfSamples.push(sample as unknown as Record<string, number | string>),
      onFirstChunkSent: () => undefined,
      onSlowChunkSnapshot: () => undefined,
      visibilitySnapshotForPlayer: () => ({ allVisible: false, visibleMask: new Uint8Array([1, 0]) }),
      cachedChunkSnapshotByPlayer: new Map(),
      fogChunkTilesByChunkKey: new Map(),
      chunkSnapshotGenerationByPlayer: new Map(),
      chunkSnapshotInFlightByPlayer: new Map(),
      ...
      summaryChunkVersion: () => 0,
      loadSummaryChunkTilesBatch: async (requests) => requests.map(({ cx, cy }) => [makeTile(cx, cy)]),
      visibleInSnapshot: (snapshot, x) => snapshot.visibleMask[x] === 1,
      wrapX: (value) => value,
      wrapY: (value) => value,
      worldWidth: 2,
      worldHeight: 1,
    ...
    controller.sendChunkSnapshot(
      { readyState: 1, OPEN: 1, send: () => undefined },
      actor,
      { cx: 0, cy: 0, radius: 0 },
      undefined,
      [{ cx: 0, cy: 0 }],
    ...
    expect(perfSamples[0]).toMatchObject({
      trigger: "fog_toggle",
      visibilityMode: "player",
      worldTiles: 2,
      worldChunks: 2,
      batchPayloadBytes: Buffer.byteLength("direct:0,0", "utf8"),
      chunkPayloadBytes: Buffer.byteLength("direct:0,0", "utf8"),
      rebuiltPayloadBytes: Buffer.byteLength("direct:0,0", "utf8"),
      playerCachePayloads: 1,
      playerVisibilityMasks: 1,
      playerVisibilitySnapshotBytes: 2,
      peakRssMb: 321,
      peakHeapUsedMb: 111
    });
  });
