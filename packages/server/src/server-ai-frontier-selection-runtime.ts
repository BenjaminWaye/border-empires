...
export const createServerAiFrontierSelectionRuntime = (
  ...
  const aiTownCompoundingStructureScore = (tileKey: TileKey, actorId: string): number => {
    const supportedTownKeys = deps.townsByTile.has(tileKey) ? [tileKey] : [];
    let bestScore = 0;
    for (const supportedTownKey of supportedTownKeys) {
      const town = deps.townsByTile.get(supportedTownKey);
      if (!town) continue;
      const connectedTownCount = town.connectedTownCount ?? 0;
      const connectedTownBonus = town.connectedTownBonus ?? 0;
