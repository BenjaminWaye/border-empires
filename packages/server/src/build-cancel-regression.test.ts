...
describe("generic build cancel regression guard", () => {
  it("routes all in-progress build cancel actions through the shared server helper", () => {
    ...
    expect(source).toContain('fort?.ownerId === actor.id && fort.status === "under_construction"');
    expect(source).toContain('observatory?.ownerId === actor.id && observatory.status === "under_construction"');
    expect(source).toContain('siege?.ownerId === actor.id && siege.status === "under_construction"');
