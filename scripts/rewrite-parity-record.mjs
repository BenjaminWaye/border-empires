...
const resolveRecordWsUrl = (raw) => {
  if (!raw) return "wss://border-empires.fly.dev/ws";
...
const recordWsUrl = resolveRecordWsUrl(process.env.RECORD_WS_URL);
...
const isTerminalEvent = (msg) =>
  // The server does not echo commandId in most responses, so we accept
  // terminal event types regardless of commandId when the server omits it.
  (msg.commandId == null || msg.commandId === pendingCommandId) &&
  ...
  new Promise(async (resolve, reject) => {
    ...
    const onMessage = (data) => {
      ...
      if (msg?.type === "ERROR") {
        ...
        ws.close?.();
        reject(new Error(`Auth/init rejected: ${msg.code ?? "ERROR"}${msg.message ? ` (${msg.message})` : ""}`));
        return;
      }
...
try {
  ...
  // If the parity player has no tiles yet, wait for the server to spawn them.
  // Spawning happens asynchronously (next game tick) after AUTH, so we poll
  // until tiles arrive or we time out.
  if (ownedTiles().length === 0) {
    process.stdout.write(`  Waiting for spawn tiles`);
    const spawnDeadline = Date.now() + 30_000;
    while (ownedTiles().length === 0 && Date.now() < spawnDeadline) {
      await new Promise((r) => setTimeout(r, 500));
      process.stdout.write(".");
    }
    process.stdout.write(ownedTiles().length > 0 ? " ✓\n" : " timed out\n");
  }
