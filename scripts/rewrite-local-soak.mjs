...
// OUT_OF_REACH — previously an unrecognized code (see the ERROR handler
// below) that aborted the whole soak iteration instead of being treated as
// just another invalid candidate to learn from.
    ...
    const onMessage = (data) => {
      ...
      if (message.type === "ERROR") {
        if (
          ...
          // Fixed-border reach (see the reach import at the top of this
          // file): computeOwnReachSet filters candidates before they're
          // ever sent, so this should be rare — but a border can shift
          // between candidate generation and this response (e.g. another
          // of the player's structures going dormant), so treat it as a
          // normal per-iteration invalid target rather than a hard abort.
          // Not marked speculative (session.speculativeInvalidTargets)
          // since reach can grow as the player's own border expands, unlike
          // BARRIER/UNKNOWN_TILE which are permanent map facts.
          message.code === "OUT_OF_REACH"
        ) {
          if (activePayload) {
